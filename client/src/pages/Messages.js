...
const Messages = (props) => {
  ...
  const socket = openSocket("https://hatchways-loving-sitter.herokuapp.com/");
  ...
  const createMessage = (e) => {
    ...
    axios
      .post(`/conversation/${conversationId}/message`, newMessage, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((res) => {
        console.log(res.data);
  ...
  const showMessage = messages.map((message, i) =>
    message.userId._id === userId ? (
      <span key={i}>
        ...
        >
          <Moment format="MMMM Do YYYY, h:mm a">{message.createdAt}</Moment>
        </Typography>
      </span>
    ) : !message.userId._id ? (
      <span key={i}>
                    ...
                    {profiles.map((item) =>
                      item.userId !== userId ? (
                        ...
                        >
                          ...
                          <Typography id={item.userId} variant="h6">
                ...
                {conversation.map((item) =>
                  item.members_info[0].userId === userId ? (
                    ...
                    >
                      ...
                      >
                        {item.members_info[1].firstName}{" "}
                        {item.members_info[1].lastName}
                      </Typography>
                    </ListItem>
                  ) : (
                    <ListItem
                    ...
                    >
                      ...
                      <Typography
                        ...
                        variant="h6"
                      >
                        {item.members_info[0].firstName}{" "}
                        {item.members_info[0].lastName}
                      </Typography>
                    </ListItem>
                  )
                )}
